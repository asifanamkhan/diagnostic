        ...
        <div class="main-card mb-3 card">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-9">
                        <form class="myForm needs-validation" method="POST" action="{{ route('service.update', $service->id) }}" novalidate>
                            @csrf
                            @method('PUT')
                            <div class="form-row">
                                <div class="col-md-6">
                            ...
                            <div class="form-row" v-for="(list, n) in form.lists" :key="n" :id="n">
                                ...
                                <div class="col-md-5">
                                    <div class="position-relative form-group">
                                        ...
                                        <input :id="'list_description' + n" placeholder="Description" type="text" class="form-control" v-model="list.description">
                                        <div class="valid-feedback">Looks Good!</div>
                                        <div class="invalid-feedback">This Field is Required</div>
                                    </div>
                                </div>
                            ...
                            <div class="form-row">
                                <div class="col-md-7"></div>
                                <div class="col-md-2">
                                    <label for="paid_amount" class=""><strong>Paid</strong></label>
                                </div>
                                <div class="col-md-3">
                                    <div class="position-relative form-group">
                                        <input id="paid_amount" placeholder="Paid" type="number" class="form-control" v-model="form.paid_amount" min="0" step="any" readonly>
                                        <span class="is-invalid" v-show="paid_amount_invalid">
                                            <strong>Paid Amount is Invalid</strong>
                                        </span>
                                        <div class="valid-feedback">Looks Good!</div>
                                        <div class="invalid-feedback">This Field is Required</div>
                                    </div>
    ...
    let app = new Vue({
        ...
        data() {
            return {
                ...
                form: {
                    ...
                    delivery_date: "{{ $service->getRawOriginal('delivery_date') }}",
                    patient_id: "{{ $service->patient_id }}",
                    status: "{{ $service->status }}",
                    doctor_id: "{{ $service->doctor_id ?? 0 }}",
                    total_amount: "{{ $service->total_amount }}",
                    discount: "{{ $service->discount }}",
                    paid_amount: "{{ $paid_amount }}",
                    due: "{{ $service->total_amount - ($paid_amount + (($service->total_amount/100)*$service->discount)) }}",
        ...
        beforeMount() {
            this.form.lists.map(object => {
                object.test_name = object.test.name;
                return object;
            });
        },
        mounted() {
        ...
        methods: {
            ...
            service() {
                this.disabled = false;
                axios.put("{{ route('service.update', $service->id) }}", this.form)
                    .then(response => {
                        Toast.fire({
                            icon: 'success',
                            title: 'Service Data Updated Successfully'
                        });
                        window.location = "{{ route('service.index') }}";
                    })
                    .catch(error => {

                        // if (error.response.data.errors.hasOwnProperty('due')) {
                        //     Toast.fire({
                        //         icon: 'success',
                        //         title: 'Service Data Updated Successfully'
                        //     });
